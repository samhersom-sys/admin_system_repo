...
| `audit_events` | user_id, entity_type, entity_id, created_at | Last-opened date field (`lastOpenedDate`) on returned records |
...
**Recommended approach:** Single page at `/search`. On mount (no filters): call `GET /api/search` with no params — backend returns the most recently created records per type (ordered by `createdDate DESC`, capped at 2,000 per type). Filter changes: rerun the same call with query params. URL param `?type=Submission` pre-selects the type dropdown on arrival.

**Potential flaws:**

1. **Default view shows all-time recent records, not personalised.** The backend always returns records ordered by `createdDate DESC`; there is no per-user "recently opened" prioritisation. New users and experienced users see the same list.
   - *Mitigation:* The `lastOpenedDate` field (sourced from `audit_event`) is still returned per record and used for the Last Opened date-range filter. A future enhancement could re-introduce personalised ordering if required.
...
3. **Large result sets in filter mode.** The backend caps filter results at 2,000 per type, so up to 14,000 rows could arrive in one response. Pagination at 50 rows/page handles this on the frontend.
...
- **Reference** — text
- **Status** — searchable single-select dropdown populated with all known record lifecycle status values (Active, Bound, Cancelled, Closed, Created, Declined, Disbanded, Expired, Lapsed, Open, Quoted, Renewed); implemented via `SearchableSelect` component in the same style as the Created By field
...
**REQ-SEARCH-FE-F-023 — Submission Type sub-filter:** The search form shall display a Submission Type filter when the authenticated user belongs to a broker organisation or holds the platform administrator role (`internal_admin`). The filter shall offer the following values: Manual, Autogenerated, Broker Submission. When selected, the filter shall be sent to `GET /api/search` as the `submissionType` query parameter and shall restrict Submission-type results to those with a matching submission type. Users who are neither a broker organisation nor a platform administrator shall not see this filter.
...
| REQ-SEARCH-FE-F-023 | `search.test.tsx` | T-SEARCH-FE-R23a / T-SEARCH-FE-R23b (pending) |
...
| 2026-05-27 | F-023 added: Submission Type sub-filter visible for broker orgs and platform admins only; `submissionType` query param sent to `/api/search` |
