...
**REQ-QUO-FE-F-046:** The Sections tab shall load section data on page mount (not gated by tab selection) and render the returned records in a `ResizableGrid`. The table shall display the following columns (matching the BackUp QuoteSections table): actions (FiPlus header to open add modal when Draft; FiSearch + FiTrash2 per row), Reference (navigation link), Class of Business, Inception Date, Expiry Date, Limit Currency, Limit Amount, Limit Loss Qualifier, Excess Currency, Excess Amount, Excess Loss Qualifier, Sum Insured Currency, Sum Insured, Premium Currency, Gross Gross Premium, Gross Premium, Deductions, Net Premium, Annual Rated GP (`annual_gross_premium`), Annual Rated NP (`annual_net_premium`), Written Order %, Signed Order %. Deferred columns (not in current QuoteSection data model — Block 3): Effective Date, Time Basis, Days on Cover, Tax Receivable. The `<thead>` and column headers shall always be visible regardless of whether there are results (per §14.7 RULE 8). When the array is empty, the `<tbody>` shall render a single row with `"No sections found."` spanning all columns.
