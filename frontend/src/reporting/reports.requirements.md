...
**REQ-RPT-BE-F-049:** The reporting semantic layer (`field-mappings.ts`) shall define a curated Measures catalog containing at minimum the following named measures: `Count of Policies`, `Count of Submissions`, `Count of Quotes`, `Count of Declined Quotes`, `Count of Renewable Quotes`, `Count of Renewed Quotes`, `Gross Net Written Premium`, `Count of Binding Authorities`, `Count of Active Policies`. Each measure shall carry a `type: 'count'` (COUNT aggregation) or `type: 'number'` (SUM aggregation) tag and an optional `filterExpr` SQL predicate for filtered counts. The backend SQL generator shall emit `SUM(CASE WHEN <filterExpr> THEN 1 ELSE 0 END)` for filtered count measures and `COUNT(*)` for unfiltered count measures. All measure labels shall be user-friendly Title Case names. Note: `Count of Lapsed Policies`, `Count of Renewed Policies`, and `Count of Non-Renewed Policies` are deferred pending OQ-RPT-007.
...
| OQ-RPT-007 | `Count of Lapsed Policies`, `Count of Renewed Policies`, `Count of Non-Renewed Policies` — what policy field and values determine these states? Current schema has status values `active`, `expired`, `cancelled`, `pending`. Does `lapsed = expired`? Is a `renewed` policy one where `renewal_date IS NOT NULL`? Is `non-renewed` an expired policy without a `renewal_date`? Awaiting confirmation before these three measures can be added to REQ-RPT-BE-F-049. | Open |
