...
| 2 | 5 core report templates (Submissions, New Business, Parties, Policies, User Login Activity) | F-008 | COVERED |
...
**REQ-RPT-FE-F-025:** The page shall expose an `"Export CSV"` action in the contextual sidebar section (alongside `Run Report`) and shall not render an inline Export button on the page body. Triggering the sidebar action shall generate a CSV file from the current result set and trigger a browser download. The filename shall be `{reportName}_{YYYY-MM-DD}.csv`.
...
**REQ-RPT-FE-F-039:** The widget editor shall support backup-aligned configuration fields by widget type. Measures and dimension attributes shall be drawn from distinct catalogs. The **Measures catalog** is a curated, named set of semantic aggregations exposed by the backend (`type: 'count'` or `type: 'number'` fields in the field-mappings semantic layer). Dimension attributes (for x-axis, grouping, legend, and table columns) are all non-measure fields. Both catalogs show the domain name in the option label. Chart widgets shall expose chart-type-specific fields: line, bar, and area charts shall allow selecting an x-axis field, x-axis label, y-axis field (rendered first, paired alongside y-axis label in the same row), y-axis label, optional legend split attribute, and one or more measures. Both x-axis and y-axis selectors shall accept any loaded measure or dimension field, including date fields such as Inception Date. If the selected y-axis field is itself a measure, it shall count as a chart measure even when the separate measures list is empty. Pie and doughnut charts shall allow selecting a category attribute, category label, and one or more measures. Metric widgets shall allow selecting a measure and aggregation. Table widgets shall allow selecting one or more attributes/columns from any domain. Text widgets shall allow entering note content. The measures selection control shall be rendered as a `<select multiple>` list (not a checkbox grid) sized to show at least 4 visible options. Triggering `Save Widgets` shall persist the configured widget definitions inside the dashboard config. The slot card shall render the edit (pen) icon and the delete (trash) icon together in the top-right corner of the slot card; there shall be no separate `Edit Widget` / `Add Widget` button at the bottom of the card. The widget configuration modal shall use a consistent width of `max-w-2xl`.

**REQ-RPT-BE-F-049:** The reporting semantic layer (`field-mappings.ts`) shall define a curated Measures catalog containing at minimum the following named measures: `Count of Policies`, `Count of Submissions`, `Count of Quotes`, `Count of Declined Quotes`, `Count of Renewable Quotes`, `Count of Renewed Quotes`, `Gross Net Written Premium`, `Count of Binding Authorities`, `Count of Active Policies`. Each measure shall carry a `type: 'count'` (COUNT aggregation), `type: 'number'` (SUM aggregation), or `type: 'ratio'` (ratio/derived aggregation, see REQ-RPT-BE-F-057) tag and an optional `filterExpr` SQL predicate for filtered counts. The backend SQL generator shall emit `SUM(CASE WHEN <filterExpr> THEN 1 ELSE 0 END)` for filtered count measures and `COUNT(*)` for unfiltered count measures. All measure labels shall be user-friendly Title Case names.

**REQ-RPT-BE-F-051:** The Measures catalog shall include the following Quote business-type measures, both sourced from the `quotes` domain: `Count of New Business Quotes` (`filterExpr: "new_or_renewal = 'New'"`) and `Count of Renewal Business Quotes` (`filterExpr: "new_or_renewal = 'Renewal'"`). Both use `quotes.new_or_renewal` (TEXT, values `'New'`/`'Renewal'`), which already exists via migration 109.

**REQ-RPT-BE-F-052:** The Measures catalog shall include the following Policy lifecycle measures in the `policies` domain: `Count of Lapsed Policies` (`filterExpr: "status = 'Expired'"`), `Count of Renewed Policies` (`filterExpr: "status = 'Renewed'"`), and `Count of Renewable Policies` (`filterExpr: "renewable = 'Renewable'"`). `Count of Lapsed Policies` maps to `policies.status = 'Expired'` (established value). `Count of Renewed Policies` requires that `'Renewed'` is a valid `policies.status` value, populated via migration 115 (adds `RENEWED` to `lookup_policy_statuses`). `Count of Renewable Policies` requires that `policies.renewable` TEXT column exists, populated via migration 114.

**REQ-RPT-DB-F-053:** (DB) A new migration (`114-alter-policies-add-renewable.js`) shall add column `renewable TEXT` to the `policies` table via `ALTER TABLE policies ADD COLUMN IF NOT EXISTS renewable TEXT`. Valid values shall be `'Renewable'` and `'Non-Renewable'` (nullable, NULL = unknown). The `policies` seed (`025-policies.js`) shall be updated to include at least two policies with `renewable: 'Renewable'` and at least one with `renewable: 'Non-Renewable'`.

**REQ-RPT-DB-F-054:** (DB) A new migration (`115-alter-lookup-policy-statuses-add-renewed.js`) shall insert `{ code: 'RENEWED', name: 'Renewed', description: 'Policy has been renewed', order_index: 6 }` into `lookup_policy_statuses` via `ON CONFLICT (code) DO NOTHING`. The `policies` seed shall be updated to include at least two policies with `status: 'Renewed'`.

**REQ-RPT-BE-F-055:** (Impact Analysis — DB) New column `policies.renewable TEXT` (migration 114) impacts: `policy.entity.ts` (add `renewable` property), `field-mappings.ts` (new `countRenewable` + `renewable` lookup field on the `policies` source), and the `policies` seed. No breaking changes to existing API contracts because the column is nullable.

**REQ-RPT-BE-F-056:** (Impact Analysis — DB) New lookup entry `RENEWED` in `lookup_policy_statuses` (migration 115) impacts: `field-mappings.ts` (`countRenewed` measure on `policies`, and `lookupValues` for the `status` field updated to include `'Renewed'`), the `policies` seed, and `policy.entity.ts` (no type change — `status` is typed `string | null`). No breaking changes to existing API contracts.

**REQ-RPT-BE-F-057:** The Measures catalog shall include a `Retention Ratio` measure on the `policies` domain. It shall have `type: 'ratio'` with `ratioNumerator: "status = 'Renewed'"` and `ratioDenominator: "renewable = 'Renewable'"`. The backend SQL generator shall emit `NULLIF(SUM(CASE WHEN <ratioNumerator> THEN 1 ELSE 0 END)::numeric, 0) / NULLIF(SUM(CASE WHEN <ratioDenominator> THEN 1 ELSE 0 END)::numeric, 0)` for `type: 'ratio'` measures. When the denominator is zero the expression returns `NULL`; the API response shall surface this as `null` for the metric value. This measure depends on both migration 114 and migration 115.

**REQ-RPT-FE-F-043:** Each `DashboardWidget` shall support an optional `filters` array of `DashboardWidgetFilter` objects (`{ field: string, operator: 'equals' | 'not_equals' | 'contains' | 'greater_than' | 'less_than', value: string }`). The widget editor modal shall render a Widget Filters section (below the main field config) with an `+ Add filter` button. Each filter row shall expose: a field select (composite key from all loaded fields), an operator select, and a value text input. Each row shall have a remove button. Filters shall be persisted on the widget definition and passed to the backend widget-data endpoint alongside dashboard-level filters. An empty filter array shall be stored when no widget filters are configured.

**REQ-RPT-FE-F-044:** Each `DashboardWidget` shall support an `ignoresDashboardFilters` boolean flag (default `false`). When `true`, the widget-data API call for that widget shall omit the dashboard-level `filters` payload, making it independent of the dashboard filter panel. The widget editor modal shall render a checkbox labelled `"Independent widget — not affected by dashboard filters"` that controls this flag. The configure-page slot card shall display a small badge or indicator when the widget has `ignoresDashboardFilters = true`, so the author can identify isolated widgets at a glance.
...
**REQ-RPT-FE-F-050:** Chart widgets shall render live data using `react-chartjs-2` (`Line` for line/area chart types, `Bar` for bar/column). The chart shall occupy `100%` of the slot area (`responsive: true`, `maintainAspectRatio: false`). The widget card shall show only the widget title — no chart-type subtitle and no separate row-label legend beneath the chart. Hover tooltips shall be provided natively by Chart.js, showing the x-label and the configured y-axis label (or measure key) alongside the formatted value. Grid lines shall be light (`rgba(0,0,0,0.04)`). The slot card container in both DashboardConfigurePage and DashboardViewPage shall clip content with `overflow-hidden`. The slot grid shall fill available viewport height using CSS Grid fractional rows (`gridTemplateRows: repeat(N, 1fr)`) with `minHeight: calc(100vh - 380px)`, where N is derived from the template's maximum row span.
...
| 2026-05-xx | F-050 added: SVG chart rendering with X/Y axis lines, circle markers for line charts, SVG rects for bar charts, axis labels, slot overflow-hidden, 160px row height. Traceability T-RPT-FE-F-R050 added. |
| 2026-05-xx | F-050 revised: chart renderer replaced with react-chartjs-2 (Line/Bar); no chart-type subtitle or legend div; Chart.js native tooltips; grid layout changed to fractional rows with minHeight calc(100vh-380px). |
| 2026-04-28 | OQ-RPT-010 resolved: measures moved to `measure_definitions` DB table (migrations 116–117, seed 032). `field-mappings.ts` dimension-only. `MeasuresModule` CRUD API created. `home.service.ts` and `reporting.service.ts` updated to load measures from DB. `filter_condition` JSONB compiler supports AND/OR logic for tenant-admin measures (no raw SQL). |
| 2026-05-xx | REQ-RPT-BE-F-051–057 added: 7 new measures across quotes and policies (Count of New Business Quotes, Count of Renewal Business Quotes, Count of Lapsed Policies, Count of Renewed Policies, Count of Renewable Policies, Retention Ratio) plus DB Impact Analysis requirements (migration 114 for policies.renewable, migration 115 for RENEWED status). OQ-RPT-007 resolved; OQ-RPT-008 (Net New Policy Count) and OQ-RPT-009 (Retention Ratio format) raised. |
