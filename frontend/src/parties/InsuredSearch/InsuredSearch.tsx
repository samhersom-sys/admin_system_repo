/**
 * InsuredSearch — click-to-open modal search for Insured parties.
 *
...
// Props
// ---------------------------------------------------------------------------
export interface InsuredSearchProps {
...
function InsuredSearchModal({ isOpen, onClose, onSelect }: ModalProps) {
    const [query, setQuery] = useState('')
    const [results, setResults] = useState<Party[]>([])
    const [loading, setLoading] = useState(false)

    // Load all insureds when modal opens
    useEffect(() => {
        if (!isOpen) return
        setQuery('')
        setLoading(true)
        listParties({ type: 'Insured' })
            .then(setResults)
            .catch(() => setResults([]))
    ...
    useEffect(() => {
        if (!isOpen) return
        if (query.length === 0) {
            // already loaded all on open; reset to full list
            setLoading(true)
            listParties({ type: 'Insured' })
                .then(setResults)
                .catch(() => setResults([]))
        ...
        setLoading(true)
        listParties({ type: 'Insured', search: query })
            .then(setResults)
            .catch(() => setResults([]))
            .finally(() => setLoading(false))
    ...
    return createPortal(
        ...
        >
            ...
            >
                ...
                <h2 className="text-lg font-semibold mb-4 pr-8">Select Insured</h2>
                ...
                {loading ? (
                    ...
                    <div className="max-h-96 overflow-y-auto border rounded">
                        <ResizableGrid
                            storageKey="table-widths-insured-search"
                            columns={[
                            ...
                            emptyMessage="No insured parties found."
...
export default function InsuredSearch({ selectedParty, onSelect, hideLabel }: InsuredSearchProps) {
    ...
    return (
        <div>
            {!hideLabel && (
                <label
                    htmlFor="insured-search-trigger"
